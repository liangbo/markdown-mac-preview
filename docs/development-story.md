...
这次开发过程也可以用一些数字来描述。需要说明的是：项目文件、提交、测试这些可以从 Git 和仓库里精确统计；token 消耗和对话轮数则依赖 Codex 会话系统的统计口径，当前本地仓库本身不会保存完整 token 账单，所以这里采用“可见会话估算 + 明确口径”的方式。

### 16.1 项目产出统计

截至这份复盘文档生成时，独立仓库里的主要数据是：
...
- 测试文件数：7 个。
- 自动化测试数：34 个。
- 最终 app 名称：`mdPreview.app`。
- 独立仓库地址：`git@github.com:liangbo/markdown-mac-preview.git`。

这些数字说明，这个项目已经不只是一次简单 demo，而是经过了多轮设计、实现、验证和整理的小型完整应用。

### 16.2 对话轮数统计

如果按“用户发出一次需求、确认、反馈或问题”算一轮，那么从最初提出“做一个 Mac 上的 Markdown 预览应用”，到完成独立仓库、PRD 和开发复盘，大约经历了 40 轮用户对话。
...
从分享角度看，这一点很重要：整个应用不是一次 prompt 生成出来的，而是通过连续反馈逐步收敛出来的。

### 16.3 Token 消耗估算

精确 token 消耗需要 Codex 平台的会话统计或账单数据。本仓库无法反推出准确 token 总数。

基于本次会话的可见长度、代码读写、测试输出、设计文档、计划文档、代码审查和多次验证输出，粗略估计：

- 如果只算用户和助手的自然语言交流，约为 4 万到 7 万 tokens。
- 如果把代码片段、diff、测试输出、工具输出和文档生成都计入，整体上下文消耗大约在 18 万到 28 万 tokens 量级。

这个数字不是计费凭证，而是分享时帮助别人理解“AI 协作开发一个完整小应用大概需要多少上下文投入”的参考口径。

更准确的说法可以这样表达：

> 这个项目大约经历了 40 轮人机协作，对话和工具调用累计消耗可能在二十万 token 量级，最终沉淀出 50 多个 Git 提交、30 多个项目文件、30 多个自动化测试，以及两份可复用的产品和开发复盘文档。
